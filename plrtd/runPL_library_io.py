...
def get_n_latest_date_fits(fits_files, n):
    """
    Finds the N FITS files with the latest 'DATE' values in their headers.
    ...
    Returns:
        list: A list of up to N paths to FITS files with the latest 'DATE' values, sorted from latest to oldest.
    """
    dated_files = []
    ...
    # Extract only file paths for the top N
    return [file for _, file in dated_files[:n]]
