...
def logit(system_keyword, status, checkfolder=True):
    ...
    if checkfolder:
        ...
        dummy = _subprocessPopen(home+"/bin/dolog "+str(system_keyword)+" '"+str(status).replace("'","")+"'", shell=True)
    else:
        # max frequency of 280Hz, will return an error if date-folder doesn't exist
        dummy = _subprocessPopen(home+"/bin/dolog -s "+str(system_keyword)+" '"+str(status).replace("'","")+"'", shell=True)
