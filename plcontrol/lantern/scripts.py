...
class LanternScripts(object):
    ...
    def plot_all_modulation_sequences(self):
        """ Use the script function called retrieve_modulation_sequence to get the the 8 first modulation sequences
        and plot them in 8 subplots. The modulation sequences are generated by the modulation.py script and are used in the plcontrol/plscripts/sequence.py script.
        """
        ...
        for sequence, axis in zip(sequences, axes.flat):
            xmod, ymod = self.retrieve_modulation_sequence(sequence)
            axis.plot(xmod, ymod, "o-")
            axis.set_aspect("equal", adjustable="box")
            axis.set_title("Sequence {} ({} points)".format(sequence, len(xmod)))
            axis.set_xlabel("X modulation")
            axis.set_ylabel("Y modulation")
